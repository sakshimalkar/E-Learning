...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($payment_id > 0 && !empty($student_name) && !empty($status)) {
        $sql = "UPDATE htmlpayment SET student_name = '$student_name', status = '$status', action = '$action_date' WHERE id = $payment_id";
        if ($conn->query($sql)) {
...
if ($payment_id > 0) {
    $sql = "SELECT * FROM htmlpayment WHERE id = $payment_id";
